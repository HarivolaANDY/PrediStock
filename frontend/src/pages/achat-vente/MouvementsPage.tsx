...
export default function MouvementsPage() {
  ...
  const filteredMouvements = allMouvements.filter(m => {
    const matchesSearch = (m.produit_name || "").toLowerCase().includes(searchTerm.toLowerCase()) ||
                          (m.reason || "").toLowerCase().includes(searchTerm.toLowerCase()) ||
                          (m.referrence || "").toLowerCase().includes(searchTerm.toLowerCase())
    const matchesType = typeFilter ? m.movement_type === typeFilter : (m.movement_type === "IN" || m.movement_type === "OUT")
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y divide-border">
              {isLoading ? (
                <tr><td colSpan={5} className="py-20 text-center"><RefreshCw className="h-6 w-6 animate-spin mx-auto mb-2 text-muted-foreground" />Chargement...</td></tr>
              ) : filteredMouvements.length === 0 ? (
                <tr><td colSpan={5} className="py-20 text-center text-muted-foreground font-medium">Aucun mouvement trouvé</td></tr>
                ...
                <tr key={m.id} className="hover:bg-muted/10 transition-colors group">
                  ...
                  <td className="px-6 py-4">
                    <div className="max-w-[240px]">
                      ...
                      <p className="text-[10px] text-muted-foreground font-mono uppercase truncate">{m.referrence || "N/A"}</p>
                    </div>
                  </td>
                </tr>
              ))}
